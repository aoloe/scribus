...
ContentPalette::ContentPalette( QWidget* parent) : ScDockPalette( parent, "ContentPalette", 0)
{
...
///	setSizePolicy( QSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum));
	...
	contentPal = new PropertiesPalette_Text( this );
	setWidget( contentPal );
...
void ContentPalette::closeEvent(QCloseEvent *closeEvent)
{
...
void ContentPalette::AppModeChanged()
{
	...
	if ((m_haveDoc) && (m_haveItem))
	{
		if (m_item->isTable())
		{
			contentPal->setEnabled(m_doc->appMode == modeEditTable);
		...
		contentPal->handleSelectionChanged();
	}
}

void ContentPalette::setCurrentItem(PageItem *i)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
...
void ContentPalette::unitChange()
{
	...
	contentPal->unitChange();
	m_haveItem = tmp;
}

void ContentPalette::NewLineMode(int mode)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
	updateGeometry();
